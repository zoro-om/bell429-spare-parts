...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const suppliedToken = context.request.headers.get("X-Setup-Token") || "";
    ...
    const configuredUsername = String(env.ADMIN_USER || "").trim();
    const requestedUsername = String(body?.username || "").trim();

    const username = configuredUsername || requestedUsername;
    ...
    await env.DB.prepare(`
      INSERT INTO users(
        id, username, password_hash, role, permissions, enabled
      )
      VALUES(?,?,?,?,?,1)
    `).bind(
      id,
