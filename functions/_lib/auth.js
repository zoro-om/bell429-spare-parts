import { ensureSecuritySchema } from "./schema.js";
...
async function loginRateKey(request, username) {
  ...
  return sha256(`${String(username).toLowerCase()}|${ip}`);
}

async function tooManyLoginFailures(env, rateKey, now) {
  ...
  `).bind("login_failed", rateKey, since).first();
...
async function recordFailedLogin(env, rateKey) {
  await env.DB.prepare(`
    ...
    VALUES(?,?,?)
  `).bind(null, "login_failed", rateKey).run();
}
...
export async function getSession(request, env) {
  ...
  const sid = cookies[SESSION_COOKIE] || cookies[OLD_SESSION_COOKIE];
  if (!sid) return null;
  ...
  const row = await env.DB.prepare(`
    ...
    WHERE s.id_hash = ?
      AND s.expires_at > ?
      AND u.enabled = 1
  `).bind(await sha256(sid), now).first();
...
export async function requireSession(request, env, csrf = false) {
  ...
  if (csrf) {
    ...
    if (suppliedHash !== session.csrf_hash) {
      return {
        response: json({ error: "CSRF validation failed" }, 403),
        session: null,
      };
...
export function hasPermission(session, permission) {
  ...
  if (session.role === "designer") return true;
...
export async function login(request, env) {
  ...
  if (await tooManyLoginFailures(env, rateKey, now)) {
    return json(
      { error: "Too many login attempts. Try again later." },
      429,
      { "Retry-After": String(LOGIN_WINDOW) }
    );
  }

  const user = await env.DB.prepare(`
    SELECT id, username, password_hash, role, permissions, enabled
    FROM users
    WHERE username = ?
  `).bind(username).first();
  ...
  const csrfToken = await deriveCsrfToken(sid);
...
export async function logout(request, env) {
  ...
  if (sid) {
    ...
    if (session) {
      await env.DB.prepare(`
        INSERT INTO audit_log(user_id, action, target_id)
        VALUES(?,?,?)
      `).bind(session.user_id, "logout", session.user_id).run();
    }
  ...
  return json(
    ...
    {
      "Set-Cookie": [
        expiredSessionCookie(SESSION_COOKIE),
        expiredSessionCookie(OLD_SESSION_COOKIE),
      ].join(", "),
    }
  );
}
...
export async function me(request, env) {
  ...
  const csrfToken = await deriveCsrfToken(session.sessionId);
  const csrfHash = await sha256(csrfToken);

  if (csrfHash !== session.csrf_hash) {
    await env.DB.prepare(`
      UPDATE sessions SET csrf_hash = ? WHERE id_hash = ?
    `).bind(csrfHash, session.id_hash).run();
  }
